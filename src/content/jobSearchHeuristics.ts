...
export function scoreCandidateLooseKeywordRelevance(
  candidate: JobCandidate,
  searchKeywords: string[]
): number {
  ...
  for (const keyword of searchKeywords) {
    const aliasGroups = buildLooseKeywordAliasGroups(keyword);
    if (aliasGroups.length === 0) {
      continue;
    }

    const matchedGroups = aliasGroups.filter((group) =>
      group.aliases.some((alias) => haystackMatchesLooseKeywordAlias(haystack, haystackTokens, alias))
    );
    if (matchedGroups.length === 0) {
      continue;
    }

    const matchedNonGenericGroup = matchedGroups.some((group) => !group.generic);
    if (
      matchedGroups.length === 1 &&
      aliasGroups.length > 1 &&
      !matchedNonGenericGroup
    ) {
      continue;
    }

    const score = Math.round((matchedGroups.length / aliasGroups.length) * 100);
    bestScore = Math.max(bestScore, score);
  }

  return bestScore >= 50 ? bestScore : 0;
}

type LooseKeywordAliasGroup = {
  aliases: string[];
  generic: boolean;
};

function buildLooseKeywordAliasGroups(keyword: string): LooseKeywordAliasGroup[] {
  const normalizedKeyword = normalizeChoiceText(keyword);
  if (!normalizedKeyword) {
    return [];
  }

  const groups: LooseKeywordAliasGroup[] = [];
  const consumedTokens = new Set<string>();
  const addGroup = (aliases: string[], generic = false) => {
    const normalizedAliases = Array.from(
      new Set(
        aliases
          .map((alias) => normalizeChoiceText(alias))
    ...
    for (const alias of normalizedAliases) {
      for (const token of alias.split(/\s+/).filter(Boolean)) {
        consumedTokens.add(token);
      }
    }
  };

  if (/\b(front\s*end|frontend)\b/.test(normalizedKeyword)) {
    addGroup([
      "frontend",
      "front end",
      "ui",
  ...
  if (/\b(back\s*end|backend)\b/.test(normalizedKeyword)) {
    addGroup([
      "backend",
      "back end",
      "platform",
  ...
  if (/\bplatform\b/.test(normalizedKeyword)) {
    addGroup([
      "platform",
      "site",
      "compute",
  ...
  if (/\b(full\s*stack|fullstack)\b/.test(normalizedKeyword)) {
    addGroup([
      "full stack",
      "fullstack",
      "software engineer",
  ...
  if (/\b(mobile|android|ios)\b/.test(normalizedKeyword)) {
    addGroup([
      "mobile",
      "android",
      "ios",
  ...
  if (/(?:^|\b)ai(?:\b|$)|\bmachine learning\b|\bml\b/.test(normalizedKeyword)) {
    addGroup([
      "ai",
      "machine learning",
      "ml",
  ...
  if (/\b(infrastructure|devops|sre|site reliability)\b/.test(normalizedKeyword)) {
    addGroup([
      "infrastructure",
      "platform",
      "deployment",
  ...
  for (const token of keywordTokens) {
    ...
    addGroup([token], BROAD_TECHNICAL_SEARCH_TOKENS.has(token));
  }

  return groups;
}

function haystackMatchesLooseKeywordAlias(
  haystack: string,
  ...
  return haystackTokens.has(alias);
}
